...
- 每条未完成任务旁边显示倒计时胶囊标签。
- 截止任务的倒计时基于 `deadline_time` 计算；持续任务的倒计时基于 `end_time` 计算；瞬时日程不显示倒计时。
...
- 已完成任务不再显示倒计时，显示“已完成”。
- 已取消任务显示“已取消”。
- 鼠标悬停在倒计时标签上时，显示详细时间信息：创建时间、截止/结束时间、剩余时间、提醒时间。

#### 逾期提示规则

- `point_event` 超过发生时间后展示为“已过”，不属于逾期。
- `deadline_task` 超过 `deadline_time` 且未完成时展示为“已逾期”。
- `duration_task` 超过 `end_time` 且未完成时展示为“已逾期”；处于开始与结束时间之间时展示为“进行中”。
...
- 时间轴按日程的主要时间从早到晚排序：瞬时日程按开始时间、截止任务按截止时间、持续任务按开始时间。
- 时间轴节点颜色规则：
  - 蓝色：今天或普通待处理任务
  - 橙色：即将到期任务
  - 红色：已逾期任务
...
判断规则：
- 有明确 start_time。
- 仅使用 start_time，不使用 end_time 或 deadline_time。
...
持续任务是独立持久化的日程时间类型，不是前端推导的展示类型，也不是业务状态。

适用场景：有明确开始时间和结束时间、需要在一段时间内持续推进的任务，例如“7/20 到 7/25 完成复盘计划”。

判断规则：
- time_type = duration_task。
- start_time 与 end_time 都不为空，且 end_time 不早于 start_time。

展示方式：
- 从 start_time 到 end_time 之间画一条较粗的竖向持续条。
- 起点显示开始日期，终点显示结束日期。
...
| time_type | varchar | 时间类型：point_event/deadline_task/duration_task |
...
> 现状说明：桌面端 Tauri、托盘、原生通知和快捷时间轴已经存在，不再是“尚未搭建”的空白模块。个人日程紧急度、疲劳评估和快捷小窗口四视图的后续工作，以[桌面端个人日程多视图与疲劳评估适配计划](./桌面端个人日程多视图与疲劳评估适配计划.md)为准；本节仅保留早期架构背景。
