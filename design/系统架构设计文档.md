...
核心规则：
- `time_type` 只允许 `point_event` / `deadline_task` / `duration_task`，并直接入库。
- `point_event` 仅使用 `start_time`；`deadline_task` 仅使用 `deadline_time`；`duration_task` 使用 `start_time + end_time`。
