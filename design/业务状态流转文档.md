...
| `time_type=deadline_task` | 截止任务 | 截止节点 + 截止卡片 |
| `time_type=duration_task` | 持续任务 | 起止节点 + 竖向持续条 |
...
- 逾期不是数据库业务状态。
- `point_event` 超过 `startTime` 后展示为“已过”，不属于逾期。
- `deadline_task` 未完成且超过 `deadlineTime` 时展示为“已逾期”。
- `duration_task` 未完成且超过 `endTime` 时展示为“已逾期”；处于 `startTime` 与 `endTime` 之间时展示为“进行中”。
- 逾期任务在同一分组内置顶。
